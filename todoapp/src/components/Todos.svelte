<script lang="ts">
  ...
  // import {fetchTodo, todos} from './store'
  ...
  $: incompleteTodos = $todos.filter((todo) => !todo.done).length
  ...
  function generateId(): string {
    const len=Number($todos[$todos.length-1].id)
    const lenn=len+1
    return lenn.toString()
    // id++;
...
let createTitle;
  ...
  function addTodo(todo: string,id:string): void {
    let newTodo: ITodo = {
      id: generateId(),
      title: todo,
      done: false,
